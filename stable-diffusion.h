...
typedef struct {
    bool return_latents;
    int init_latent_index;
    int init_key;
    TensorType init_type;
} extra_inference_parameters;

SD_API sd_ctx_t* new_sd_ctx(
...
typedef enum TensorType TensorType;
SD_API bool vae_process(sd_ctx_t* sd_ctx, int key, bool decode);
SD_API bool convert_to_tensors(uint8_t** images, int width, int height, int count);
