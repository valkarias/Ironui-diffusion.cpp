#ifndef STABLE_DIFFUSION_SHARED_H
#define STABLE_DIFFUSION_SHARED_H
...
class StableDiffusionGGML {
    public:
        ...
        // Model components stored in a hashmap
        std::unordered_map<std::string, std::shared_ptr<void>> modelComponents;
        ...
        ~StableDiffusionGGML() {
            ...
            LOG_DEBUG("StableDiffusionGGML destructor called");
        }
        ...
        // Methods to set and get model components
        template <typename T>
        void setModelComponent(const std::string& key, const std::shared_ptr<T>& component) {
            modelComponents[key] = std::static_pointer_cast<void>(component);
        }

        template <typename T>
        std::shared_ptr<T> getModelComponent(const std::string& key) const {
            auto it = modelComponents.find(key);
        ...
        // Method to remove a model component by name
        void removeModelComponent(const std::string& key) {
            modelComponents.erase(key);
        }
    
};
    
#endif  // STABLE_DIFFUSION_SHARED_H